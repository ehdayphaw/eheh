    ...
    <div class="container">
      ...
      <div class="main_contents">
		...
		<form id="form"  action="addcourse" method="post" th:object="${courseBean}" onsubmit="return toSubmit()">
            ...
            <tbody>
            <tr th:each="course, iterStat : ${list}">
            		<th scope="row" th:text="${iterStat.index + 1}"></th>
            		<td th:text="${course.id}" />              
                    <td th:text="${course.name}" />
                    <td th:text="${course.action}" th:style="${course.action == 'Active' ? 'color: #00c756;' : 'color: red;'}" />
                	
                	<td style="white-space: nowrap;">
					    
					    <form th:action="@{/deletecourse1}" method="get" style="display: inline-block;">
					        <input type="hidden" name="id" th:value="${course.id}" />
					        <button type="submit" class="btn btn-success" data-bs-toggle="modal" data-bs-target="#exampleModal">Active</button>
					    </form>
					    
					    <form th:action="@{/deletecourse}" method="get" style="display: inline-block; margin-right: 10px;">
					        <input type="hidden" name="id" th:value="${course.id}" />
					        <button type="submit" class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#exampleModal">Inactive</button>
					    </form>				    
					</td>
              </tr>
